...
| `Heatmap`   | `heatmap.go`    | `NewHeatmap(id, class, rows, cols int)`                    | `SetValue(row, col, v)`, `SetRow(row, vs)`, `SetAll(vs)`, `SetRowLabels`, `SetColLabels`, `SetCellWidth(int)` — colour-graded activity grid; interpolates bg between `"heatmap/zero"` and `"heatmap/max"` |
