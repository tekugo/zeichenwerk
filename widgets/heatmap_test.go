...
func newHeatmapRenderer() (*TestScreen, *Renderer) {
	cs := NewTestScreen()
	return cs, NewRenderer(cs, NewTheme())
}

func renderHeatmap(h *Heatmap, w, ht int) *TestScreen {
...
func TestHeatmap_MidCell_IsInterpolated(t *testing.T) {
	...
	fg := cs.Fg(0, 0) // col 0 (the mid cell)
	if fg != "#808080" {
...
func TestHeatmap_RowLabels_Offset(t *testing.T) {
	...
	if cs.Get(0, 0) != "A" {
		t.Errorf("row label at (0,0)=%q; want A", cs.Get(0, 0))
	}
	// Cell at (2,0) (after lcw=2) should use cell fg
	if cs.Fg(2, 0) != "cell-fg" {
		t.Errorf("cell (2,0) fg=%q; want cell-fg", cs.Fg(2, 0))
	}
}
...
func TestHeatmap_ColLabels_Offset(t *testing.T) {
	...
	if cs.Get(0, 0) != "X" {
		t.Errorf("col label at (0,0)=%q; want X", cs.Get(0, 0))
	}
	if cs.Get(2, 0) != "Y" {
		t.Errorf("col label at (2,0)=%q; want Y", cs.Get(2, 0))
	}
	// Data row should be at y=1
	if cs.Fg(0, 1) != "cell-fg" {
		t.Errorf("cell (0,1) fg=%q; want cell-fg", cs.Fg(0, 1))
	}
}
