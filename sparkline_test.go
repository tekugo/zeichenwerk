...
// cellScreen records every Put call so tests can inspect rendered characters.
...
func renderSparkline(s *Sparkline, w, h int) *cellScreen {
...
// bottom (row 0 = top).
func colChars(cs *cellScreen, col, h int) []string {
	out := make([]string, h)
	for row := 0; row < h; row++ {
		ch := cs.cells[[2]int{col, row}]
		if ch == "" {
			ch = " "
		}
		out[row] = ch
...
func TestSparkline_Threshold_HighStyleApplied(t *testing.T) {
	...
	lowFG := cs.fgs[[2]int{0, 0}]
	highFG := cs.fgs[[2]int{1, 0}]
	if lowFG != "low-fg" {
...
func TestSparkline_Gradient_AllRowsSameColor(t *testing.T) {
	...
	fg0 := cs.fgs[[2]int{0, 0}]
	fg1 := cs.fgs[[2]int{0, 1}]
	fg2 := cs.fgs[[2]int{0, 2}]
	if fg0 != fg1 || fg1 != fg2 {
