...
func TestTheme_Apply_SetsBaseStyle(t *testing.T) {
	theme := NewTheme()
	theme.SetStyles(NewStyle("button").WithColors("white", "blue"))
	btn := NewButton("b", "", "OK")
	theme.Apply(btn, btn.Selector("button"))
	// After Apply, the widget's default style should be set
	style := btn.Style()
	if style == nil {
		t.Error("Style() should not be nil after theme.Apply()")
	}
}

func TestTheme_Apply_SetsStateStyle(t *testing.T) {
	theme := NewTheme()
	theme.SetStyles(
		NewStyle("button").WithColors("white", "blue"),
		NewStyle("button:focused").WithColors("black", "cyan"),
	)
	btn := NewButton("b", "", "OK")
	theme.Apply(btn, btn.Selector("button"), "focused")
	btn.SetFlag(FlagFocused, true)
	style := btn.Style()
	if style == nil {
		t.Error("Style() should not be nil when focused after theme.Apply()")
	}
}
